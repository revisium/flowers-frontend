...
export const BlogExperimentCard = ({ entry, text }: BlogExperimentCardProps) => {
  ...
  return (
    <Grid
      ...
      borderRadius="12px"
      gridTemplateColumns={{ base: '1fr', lg: 'minmax(320px, .92fr) minmax(0, 1.08fr)' }}
      overflow="hidden"
    >
      <Image alt={entry.imageAlt} height="100%" minHeight={{ base: '300px', lg: '410px' }} objectFit="cover" src={entry.image} width="100%" />
      <Flex alignItems="flex-start" direction="column" justifyContent="center" padding={{ base: '30px 22px', md: '38px' }}>
        ...
        <Grid gap="14px" gridTemplateColumns="repeat(3, minmax(0, 1fr))" marginTop="28px" width="100%">
          {entry.stats.map(([value, label]) => (
            <Flex borderLeft="1px solid #e3ddcf" direction="column" key={label} paddingLeft="13px">
