...
const secondaryCare = {
  en: [
    ...
    [
      'Feeding',
      'Use a balanced fertiliser during active growth, following the product instructions.',
    ],
    [
      'Support & shaping',
  ...
  ru: [
    [
      'Грунт',
      'Гумусный и воздухопроницаемый субстрат, который удерживает немного влаги, но хорошо дренируется.',
    ],
    ['Пересадка', 'Пересаживайте весной, когда корни полностью освоят горшок.'],
    [
      'Подкормки',
      'В период активного роста используйте сбалансированное удобрение по инструкции производителя.',
    ],
    [
      'Опоры и формировка',
    ...
    >
      ...
      >
        {profile.facts.map((fact, index) => (
          <Box
            {...getFactCellDividerStyles(index)}
      ...
      >
        ...
        <QuickFact
          ...
          value={profile.quickFacts.height[locale]}
        />
        ...
        <QuickFact
          ...
          value={profile.quickFacts.temperature[locale]}
        />
      </Grid>
      ...
      >
        {secondaryCare[locale].map(([title, body], index) => (
          <CareCard body={body} icon={['♧', '▣', '▤', '▦'][index]!} key={title} title={title} />
        ))}
      </Grid>
      ...
      <Grid
        ...
        marginTop="12px"
      >
        <FooterCard icon="♧" title={text.propagation}>
          {footer.propagation}
        </FooterCard>
        <FooterCard icon="⚠" title={text.problems}>
          {footer.problems.map((item) => (
            <Text key={item}>❧ {item}</Text>
          ))}
        </FooterCard>
        <FooterCard icon="♧" title={text.facts}>
      ...
      <Grid gap="1px" gridTemplateColumns={{ base: '1fr', md: '0.85fr 1.15fr' }} marginTop="10px">
        <FooterCard accent icon="!" title={text.important}>
          {footer.important}
        </FooterCard>
        <FooterCard title={text.notes}>
          <Box borderBottom="1px dashed #d8c9b3" height="22px" />
          <Box borderBottom="1px dashed #d8c9b3" height="22px" />
          <Box borderBottom="1px dashed #d8c9b3" height="22px" />
        </FooterCard>
      </Grid>
...
const QuickFact = ({
  ...
  <Flex
    ...
    textAlign="center"
  >
    <Text color="#788d52" fontSize="1.3rem">
      {icon}
    </Text>
...
const CareCard = ({
  ...
  <Flex
    ...
    padding="16px"
  >
    <Text color="#728b50" fontSize="1.35rem">
      {icon}
    </Text>
  ...
  <Flex gap="5px">
    {Array.from({ length: 6 }, (_, index) => (
      <Image
        ...
        src={
          index < level
            ? '/plant-profile/difficulty-leaf-filled.png'
            : '/plant-profile/difficulty-leaf-outline.png'
        }
        width="25px"
      />
    ))}
